...
### 9.4 Empirical Data: The River Scaling Effect

The following data was collected from the author's Qiskit experiment framework, comparing decoherence patterns across system sizes ($N=4, 6, 8$) and topologies (GHZ, W, Cluster, Product).

**Methodological notes:**
- **Shot Counts**: Different experiments use different shot counts (2048–8192). Since PCR, EEC, AI, and entropy operate on normalized distributions, comparisons are valid across shot counts. The raw histograms are included purely for transparency.
- **Metric**: Pathway Concentration Ratio (PCR) measures how "unequal" the error distribution is. A random distribution has PCR $\approx 1$.
- **Metric**: Entanglement-Error Correlation (EEC) measures how well error locations correlate with the state's entanglement graph.
- **Noise**: Amplitude Damping ($\gamma=0.1$) unless otherwise specified. *(We choose $\gamma=0.1$ and $p=0.1$ because they produce comparable single-qubit state-fidelity reductions (~10% deviation), ensuring a fair comparison between directional and isotropic noise.)*

#### 9.4.1 The "River Scaling" Phenomenon (GHZ State)

We tested the hypothesis that larger entangled systems concentrate errors more effectively. *For reference, a perfectly uniform distribution over $2^n$ basis states yields PCR = 1.0. Values significantly above 1 indicate structured concentration.*

| System Size | PCR (Concentration) | EEC (Correlation) | Interpretation |
|:---|:---|:---|:---|
| **4 Qubits** | ~212 | ~0.40 | Strong concentration. |
| **6 Qubits** | **520.1** | 0.58 | **Peak Concentration**. The "River" deepens. |
| **8 Qubits** | 401.9 | **0.62** | **Peak Correlation**. Entropy fights concentration, but structure strengthens. |

**Finding**: The "River" effect scales non-monotonically in concentration (peaking at 6q) but **monotonically in correlation**. For the GHZ series, EEC rises from 0.40 (4 qubits) → 0.58 (6 qubits) → 0.62 (8 qubits), indicating that correlation structure strengthens with system size even when PCR slightly declines. As the system grows, the errors follow the entanglement topology more strictly, even as the sheer number of pathways ($2^8$) dilutes the concentration slightly.

#### 9.4.2 Topology Hierarchy: Rivers, Streams, and Puddles

We compared 6-qubit states under identical noise ($\gamma=0.1$) to isolate the role of entanglement topology.

| State Type | Topology | PCR | EEC | Metaphor |
|:---|:---|:---|:---|:---|
| **GHZ** | Global (All-to-All) | **520.1** | **0.58** | **The River**: Deep, channeled flow. |
| **Cluster** | Local (1D Chain) | 4.9 | ~0.00 | **The Stream**: Shallow, local concentration. |
| **Product** | None (Separable) | 1.96 | 0.00 | **The Fog**: Random independent errors. |
| **W State** | Distributed | 1.05 | 0.00 | **The Puddle**: Uniform spreading. |

**Finding**: Global entanglement (GHZ) is required for the "River" effect. Local entanglement (Cluster) produces only weak concentration.

#### 9.4.3 The Null Hypothesis: Structure Requires Structured Noise

We compared 6-qubit GHZ states under Structured Noise (Amplitude Damping) vs. Random Noise (Depolarizing).

| Noise Model | PCR | Interpretation |
|:---|:---|:---|
| **Amplitude Damping** | **520.1** | **Active Channeling**: Noise drives system into the river. |
| **Depolarizing** | 261.1 | **Passive Survival**: Peaks survive, but errors scatter. |

**Finding**: Structured noise **doubles** the concentration effect. The "River" is formed by the *interaction* of state topology and noise geometry, not by state topology alone.

#### 9.4.4 Pathway Flow Visualization

```
    THE RIVER (GHZ + Amp Damp)             THE FOG (GHZ + Depolarizing)
    (Deep, Channeled Flow)                 (Isotropic Scattering)
 ...
 |01111111⟩...|11111110⟩             |01111111⟩...|11111110⟩
  (Weight 7 states)                    (Scatters to all)
   │      │      │                      │   │    │    │
   └──────┼──────┘                      ▼   ▼    ▼    ▼
          ▼                          (Uniform leakage)
 |00111111⟩...|11111100⟩            (to all 256 states)
  (Weight 6 states)                     │   │    │    │
          │                             ▼   ▼    ▼    ▼
          ▼                          |00000000⟩   |11111111⟩
 (Cascades w=5...w=1)               (Both poles preserved)
          │
          ▼
       |00000000⟩
   (Absorbing state)
    ...
    THE STREAM (Cluster + Amp Damp)        THE PUDDLE (W + Amp Damp)
    (Shallow, Local Flow)                  (Robust, Flat Spread)

       |Cluster_Init⟩                         |W_N State⟩
          │                                      │
     ┌────┼────┐                                 ▼
     ▼    ▼    ▼                      [ |10..0⟩ ... |00..1⟩ ]
   (Many weak paths)                     (Uniform "Puddle")
     │    │    │                                 │
     ▼    ▼    ▼                                 ▼
     |00...00⟩                               |00...00⟩
   (Low concentration)                    (Slow drainage)
```

**Skeptic's Rebuttal: Is this just trivial amplitude damping?**
One might argue that since amplitude damping drives every qubit to $|0\rangle$, the flow towards $|00\dots0\rangle$ is trivial. However, while the *destination* is fixed by the noise, the *pathway* is determined by the entanglement. A product state decays via a uniform cascade; the GHZ state restricts flow to a structured "River" (high PCR, high EEC). The novelty is not the decay itself, but the *collective geometry* of the intermediate states.

#### 9.4.5 Visualizing the Flow: 6-Qubit Data

**Chart A: The River (GHZ-6 + Amplitude Damping)**
*Raw counts (8192 shots). Note the directional flow from |111111⟩ to |000000⟩ via high-Hamming-weight intermediates.*
...
**Chart B: The Fog (GHZ-6 + Depolarizing)**
*Raw counts (8192 shots). Note the preservation of both poles and isotropic scattering.*
...
**Chart C: The Stream (Cluster-6 + Amplitude Damping)**
*Raw counts (8192 shots). Note the lack of strong concentration compared to GHZ (The River).*
...
**Chart D: The Puddle (W-6 + Amplitude Damping)**
*Raw counts (8192 shots). Note the uniform distribution across the single-excitation subspace, showing high robustness (low concentration).*
...
*Note: The River effect emerges only when entanglement topology and noise geometry align; neither state topology alone nor noise geometry alone is sufficient. This aligns precisely with the Structured Substrate Thesis (SST).*

#### 9.4.6 Interpretation

The data confirms the SST prediction that decoherence is the product of **interaction geometry**:

1. **The River (GHZ + Amp Damp)**: When global entanglement meets directional noise, the system collapses along a narrow, highly concentrated channel. This is the "River" effect (High PCR, High EEC).

2. **The Fog (GHZ + Depolarizing)**: When the same state meets isotropic noise, the constraints are overwhelmed by spherical scattering. The structure dissolves into a high-entropy "Fog" (Low PCR).

3. **The Stream (Cluster + Amp Damp)**: Local entanglement (1D chain) provides weaker constraints. The noise creates many shallow, local runoffs rather than a single deep channel. The result is a "Stream" (Low PCR, Low EEC).

4. **The Puddle (W + Amp Damp)**: The W-state's distributed single-excitation symmetry resists concentration. Even under directional noise, the probability spreads uniformly across the subspace, forming a robust "Puddle" (PCR $\approx$ 1).

**Conclusion**: Structure requires both **state topology** and **noise geometry** to align.
- **GHZ** is brittle but channel-forming.
- **W** is robust but diffusive.
- **Cluster** is intermediate.
