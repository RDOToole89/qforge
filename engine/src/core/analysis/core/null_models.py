...
def _depolarizing_transition_prob(
    outcome_bit: int, basis_bit: int, effective_error_rate: float
) -> float:
    """Single-qubit depolarizing transition probability.

    For depolarizing noise with effective per-qubit flip rate ε:
        P(same | input) = 1 - ε
        P(flip | input) = ε

    Args:
        outcome_bit: Measured bit (0 or 1)
        basis_bit: Ideal input bit (0 or 1)
        effective_error_rate: Per-qubit effective flip probability ε

    Returns:
        Transition probability.
    """
    if outcome_bit == basis_bit:
        return 1.0 - effective_error_rate
    return effective_error_rate


def state_aware_null_model(
    state_type: str,
    num_qubits: int,
    error_rate: float,
    noise_type: str = "depolarizing",
    gate_counts_per_qubit: dict[int, int] | None = None,
    state_vector: Optional[np.ndarray] = None,
) -> dict[str, float]:
    """
    Null model that predicts measurement outcomes for *independent* noise
    applied to a *known* quantum state.

    Unlike the factorized null (which ignores the ideal state), this model
    computes:
        P(outcome) = Σ_basis |⟨basis|ψ⟩|² × Π_i P_noise(outcome_i | basis_i, ε_i)

    This is the proper null for testing whether decoherence has *excess*
    structure beyond what independent per-qubit noise on the known state predicts.

    Args:
        state_type: Quantum state type (e.g. "GHZ", "W", "CLUSTER").
        num_qubits: Number of qubits.
        error_rate: Base depolarizing error probability p per gate.
        noise_type: Currently only "depolarizing" is supported.
        gate_counts_per_qubit: If provided, per-qubit gate counts for
            accumulated noise calculation. If None, assumes balanced
            (one gate per qubit).
        state_vector: Explicit state vector. If None, obtained from
            create_state_instance().

    Returns:
        Normalised probability distribution over full 2^n support
        in canonical lexicographic order.
    """
    if noise_type != "depolarizing":
        raise ValueError(
            f"state_aware_null_model only supports 'depolarizing' noise, got '{noise_type}'"
        )

    # Obtain ideal state vector
    if state_vector is None:
        from src.core.state_preparation.state_factory import create_state_instance

        state_instance = create_state_instance(state_type, num_qubits)
        state_vector = state_instance.get_theoretical_state_vector()

    state_vector = np.asarray(state_vector, dtype=np.complex128)

    # Compute per-qubit effective error rates
    # For depolarizing noise, each gate introduces a bit-flip probability of 2p/3.
    # After k gates: ε = 1 - (1 - 2p/3)^k
    per_qubit_epsilon = np.zeros(num_qubits, dtype=np.float64)
    single_gate_flip = 2.0 * error_rate / 3.0

    if gate_counts_per_qubit is not None:
        for q in range(num_qubits):
            k = gate_counts_per_qubit.get(q, 1)
            per_qubit_epsilon[q] = 1.0 - (1.0 - single_gate_flip) ** k
    else:
        # Balanced assumption: one effective gate per qubit
        for q in range(num_qubits):
            per_qubit_epsilon[q] = single_gate_flip

    # Find basis states with nonzero amplitude
    amplitudes_sq = np.abs(state_vector) ** 2
    nonzero_mask = amplitudes_sq > 1e-15
    nonzero_indices = np.where(nonzero_mask)[0]

    # Canonical lexicographic bitstring order
    order = all_bitstrings(num_qubits)

    # Build null distribution
    null_model: dict[str, float] = {}
    for outcome_bs in order:
        prob = 0.0
        for basis_idx in nonzero_indices:
            amp_sq = float(amplitudes_sq[basis_idx])
            # Convert basis index to bitstring
            basis_bs = format(basis_idx, f"0{num_qubits}b")

            # Product of per-qubit transition probabilities
            transition_prob = 1.0
            for i in range(num_qubits):
                transition_prob *= _depolarizing_transition_prob(
                    int(outcome_bs[i]), int(basis_bs[i]), per_qubit_epsilon[i]
                )

            prob += amp_sq * transition_prob
        null_model[outcome_bs] = prob

    # Renormalize defensively
    Z = sum(null_model.values())
    if not np.isfinite(Z) or Z <= 0.0:
        raise RuntimeError("State-aware null normalization failed.")
    for k in null_model:
        null_model[k] /= Z

    logger.debug(
        f"State-aware null built: state_type={state_type}, n={num_qubits}, "
        f"error_rate={error_rate}, gate_counts={gate_counts_per_qubit}"
    )
    return null_model
