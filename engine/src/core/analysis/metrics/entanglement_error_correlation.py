...
class PermutationTestResult:
    """Result of a permutation test for EEC significance."""
    ...
    n_permutations: int
    significant: bool
    effect_size: float  # (observed - mean_null) / std_null


def eec_permutation_test(
    counts: Mapping[str, int],
    state_type: str = "GHZ",
    n_permutations: int = EEC_PERMUTATION_N,
    alpha: float = EEC_SIGNIFICANCE_ALPHA,
    rng: np.random.Generator | None = None,
) -> PermutationTestResult:
    """
    Permutation test for EEC significance.

    Tests whether the observed EEC is significantly different from what
    would be expected under random qubit-label permutations of the
    topology matrix.

    For small n (n! <= n_permutations), uses exhaustive enumeration.
    For large n, uses random sampling.

    Args:
        counts: Measurement counts {bitstring: count}
        state_type: Quantum state topology
        n_permutations: Number of random permutations (ignored if exhaustive)
        alpha: Significance threshold
        rng: Random generator for reproducibility

    Returns:
        PermutationTestResult with observed EEC, p-value, and null distribution
    """
    ...
    counts_clean = validate_counts_dict(counts, "EEC permutation test input")
    first_bs = next(iter(counts_clean.keys()))
    n_qubits = len(first_bs)

    if n_qubits < 2:
        return PermutationTestResult(
            observed=0.0, p_value=1.0, null_distribution=[0.0],
            n_permutations=0, significant=False, effect_size=0.0,
        )

    # Build matrices
    W = _construct_entanglement_topology(n_qubits, state_type, {})
    E = _compute_error_correlation_matrix(counts_clean, n_qubits, ALPHA)

    # Observed EEC
    observed_eec, _ = _compute_topology_error_correlation(W, E)

    # Decide exhaustive vs random
    n_fact = factorial(n_qubits)
    exhaustive = n_fact <= n_permutations

    null_eecs: list[float] = []

    if exhaustive:
        for perm in iter_permutations(range(n_qubits)):
            perm_arr = np.array(perm)
            W_perm = W[np.ix_(perm_arr, perm_arr)]
            eec_perm, _ = _compute_topology_error_correlation(W_perm, E)
            null_eecs.append(eec_perm)
    else:
    ...
    # p-value: fraction of null EECs >= observed
    null_array = np.array(null_eecs)
    p_value = float(np.mean(null_array >= observed_eec))

    # Effect size
    null_mean = float(np.mean(null_array))
    null_std = float(np.std(null_array))
    if null_std > 1e-12:
        effect_size = (observed_eec - null_mean) / null_std
    else:
        effect_size = 0.0

    return PermutationTestResult(
        observed=observed_eec,
        p_value=p_value,
        null_distribution=null_eecs,
        n_permutations=len(null_eecs),
