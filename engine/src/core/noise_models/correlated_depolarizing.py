...
Comparing EEC across these three conditions is the definitive simulation test.
"""
...
# Topology adjacency matrices for known state types
_TOPOLOGY_BUILDERS: dict[str, Any] = {}


def _ghz_topology(n: int) -> np.ndarray:
    """GHZ: chain connectivity (H-CX-CX-...) edges."""
    W = np.zeros((n, n))
    for i in range(n - 1):
        W[i, i + 1] = W[i + 1, i] = 1.0
    return W
...
def _w_topology(n: int) -> np.ndarray:
    """W: all-to-all symmetric connectivity."""
    W = np.ones((n, n)) - np.eye(n)
    return W


def _cluster_topology(n: int) -> np.ndarray:
    """Cluster: linear chain H+CZ connectivity."""
    W = np.zeros((n, n))
    for i in range(n - 1):
        W[i, i + 1] = W[i + 1, i] = 1.0
    return W
...
class CorrelatedDepolarizingNoise(BaseNoise):
    ...
    def __init__(
        ...
        Args:
            ...
            topology: State topology name ("GHZ", "W", "CLUSTER").
        ...
        elif self.topology_name in _TOPOLOGY_BUILDERS:
            self.topology_matrix = _TOPOLOGY_BUILDERS[self.topology_name](num_qubits)
        else:
            raise ValueError(
                f"Unknown topology '{topology}'. "
                f"Provide custom_topology or use one of: {list(_TOPOLOGY_BUILDERS)}"
            )
    ...
    def apply(
        ...
        - Single-qubit gates get standard depolarizing noise (same as DepolarizingNoise).
    ...
    def _build_correlated_2q_channel(self) -> Any:
        ...
        For cs >= 0 (correlated bias):
            ...
            - Standard: each of 15 Paulis gets p/15
            - Correlated only: XX, YY, ZZ each get p/3
        ...
        For cs < 0 (anti-correlated bias), let t = |cs|:
            ...
            - Standard: each of 15 Paulis gets p/15
            - Uncorrelated only: 12 non-correlated Paulis each get p/12
        ...
        if cs >= 0:
            # Mix standard depol with purely correlated channel
            corr_prob = (1 - cs) * p / 15.0 + cs * p / 3.0
            uncorr_prob = (1 - cs) * p / 15.0
        else:
            # Mix standard depol with purely uncorrelated channel
            t = abs(cs)
    ...
    def get_kraus_operators(self) -> list[np.ndarray]:
        ...
        channel = self._build_correlated_2q_channel()
        # Return the Pauli probabilities as a proxy
        return []
