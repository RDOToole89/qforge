...
- The `metrics_bundle` field is only present if `config.metrics` is not None
  and counts were extracted.
...
def run(
    ...
    5) Optionally compute analysis metrics (via MetricsBundle).
    ...
    storage = LocalStorage(base_dir=ctx.base_results_dir, session_id=getattr(ctx, 'session_id', None))
    ...
    if (
        ...
        try:
            ...
            json_stem = os.path.splitext(os.path.basename(saved_path))[0]
            hist_path = os.path.join(os.path.dirname(saved_path), f"{json_stem}_histogram.png")
