...
| `metrics` | str/list/None | "structured_decoherence" | Profile name, metric list, or None |
...
# Metrics bundle (when metrics are enabled)
bundle = result.metrics_bundle
for name, entry in bundle.metrics.items():
    print(f"{name}: {entry.value:.4f} ({entry.status})")
```
...
ai_values = [r.metrics_bundle.value("structure_score") for r in results]
...
When `metrics` is set (e.g., `metrics="structured_decoherence"`), the following metrics are computed:
...
**"No metrics_bundle"**
- Ensure `metrics` is set in config (e.g., `metrics="structured_decoherence"`)
- Check that noise is enabled and counts are being produced
