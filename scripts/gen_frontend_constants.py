...
from qforge.engine.models.config import EXPERIMENT_TYPE_EXAMPLES, ExperimentConfig
...
def _build_ts() -> str:
    ...
    try:
        ...
        # Free string label — catalog lists suggested examples only.
        experiment_types = list(EXPERIMENT_TYPE_EXAMPLES)
    ...
    for sid in state_ids:
        ...
        state_rows.append(f'  {{ id: "{sid}", minQubits: {n_min}, maxQubits: {n_max} }},')
    ...
    for fid, (stype, n, params) in _NAMED_STATEVECTORS.items():
        ...
        sv_rows.append(
            f'  {fid}: {{ id: "{fid}", numQubits: {n}, amplitudes: {_complex_array(amps)} }},'
        )
    ...
    for key, (stype, n) in _CORRELATOR_STATES.items():
        ...
        corr_rows.append(f"  {key}: {{ numQubits: {n}, correlators: {{ {body} }} }},")
...
def _verify_against_ideal_states() -> None:
    ...
    all_pass = True
    for fid, (stype, n, params) in _NAMED_STATEVECTORS.items():
        gen = np.array([complex(re, im) for re, im in _frontend_statevector(stype, n, params)])
