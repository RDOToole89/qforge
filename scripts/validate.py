...
class ValidationResults:
    ...
    def add_error(
        ...
        self.success = False
        self.errors.append(
            ValidationError(file=file, message=message, details=details, kind=kind)
        )
        if fail_fast:
            raise SystemExit(1)
...
class SchemaValidator:
    ...
    def __init__(self, repo_root: Path, fail_fast: bool = False):
        ...
        self.schema_registry: Dict[str, Any] = (
            {}
        )  # key: schema_id or relative path string
    ...
    def load_schema_index(self) -> bool:
        ...
        if (
            not isinstance(self.schema_index, dict)
            or "schemas" not in self.schema_index
        ):
    ...
    def validate_schemas(
        self, results: ValidationResults, only_schema_id: Optional[str] = None
    ) -> bool:
    ...
    def validate_cross_references(
        ...
        for sid, path in self.schema_paths.items():
            try:
        ...
        for sid, schema in list(self.schema_registry.items()):
            ...
            for ref_path, ref_target in refs:
                ...
                if ref_target.startswith("http://") or ref_target.startswith(
                    "https://"
                ):
    ...
    def create_resolver(self) -> RefResolver:
        ...
        resolver = RefResolver(
            base_uri=self.schemas_dir.as_uri() + "/", referrer={}, store=store
        )
        return resolver
...
class ExampleValidator:
    ...
    def validate_examples(
        ...
        only_schema_id: Optional[str] = None,
    ):
    ...
    @staticmethod
    def _extract_schema_id(filename: str) -> Optional[str]:
...
def validate_artifacts_directory(
    ...
    candidate_items: Iterable[Tuple[str, dict]] = (
        schema_validator.schema_registry.items()
    )
    if only_schema_id:
        if only_schema_id not in schema_validator.schema_registry:
            ...
            return
        candidate_items = [
            (only_schema_id, schema_validator.schema_registry[only_schema_id])
        ]
...
def print_human_results(results: ValidationResults) -> None:
    ...
    if results.artifacts_total:
        print(
            f"🧪 Artifacts: {results.artifacts_valid}/{results.artifacts_total} valid"
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Validate Quantum Experiment Schema Suite"
    )
    parser.add_argument(
        "--schemas-only", action="store_true", help="Validate schemas only"
    )
    parser.add_argument(
        "--examples-only", action="store_true", help="Validate examples only"
    )
    parser.add_argument(
        "--output",
        choices=["human", "json"],
    ...
    try:
        ...
        if not args.examples_only:
            ...
            schema_validator.validate_cross_references(
                results, only_schema_id=args.schema_id
            )
