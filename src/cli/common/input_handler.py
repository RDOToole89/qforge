# src/cli/common/input_handler.py
...
class InputHandler:
        ...
        **kwargs,
        ...
        while True:
            try:
                ...
                # In non-TTY environments, auto-accept default to avoid EOF
                try:
                    import sys as _sys
    ...
    def select_option(
        ...
        auto_assign_unique_hotkeys: bool = True,
    ) -> str:
        ...
        # Optionally auto-assign unique hotkeys to avoid collisions
        if auto_assign_unique_hotkeys:
            normalized = self._assign_unique_hotkeys(normalized)
    ...
    @staticmethod
    def _assign_unique_hotkeys(
        normalized_options: list[tuple[str, str, str]],
    ) -> list[tuple[str, str, str]]:
        """Ensure each option has a unique, mnemonic hotkey.

        Strategy: use existing hotkeys when unique; fill or fix collisions by
        picking the first unused alphanumeric character from label.
        """
        assigned = set()
        result = []
        # First pass: keep unique existing
        for value, label, hotkey in normalized_options:
            hk = (hotkey or "").strip().lower()
            if hk and hk not in assigned and len(hk) == 1 and hk.isalnum():
                assigned.add(hk)
                result.append((value, label, hk))
            else:
                result.append((value, label, ""))
        # Second pass: assign missing
        for i, (value, label, hotkey) in enumerate(result):
            if hotkey:
                continue
            candidate = None
            for ch in label.lower():
                if ch.isalnum() and ch not in assigned:
                    candidate = ch
                    break
            if candidate is None:
                # fallback to digits 0-9
                for ch in "1234567890":
                    if ch not in assigned:
                        candidate = ch
                        break
            result[i] = (value, label, candidate or "")
            if candidate:
