...
from .collectors import ParameterCollector
from .presets_browser import PresetsBrowser
...
class Router:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if choice == "1":
                ...
                try:
                    args = self.browser.browse(include_keys=[
                        "ghz_basic",
                        "ghz_noise",
                        "density_analysis",
                ...
                try:
                    args = self.collector.collect_parameters(interactive=True, force_state_type="CUSTOM")
                except KeyboardInterrupt:
                    continue
            elif choice == "4":
                ...
                sub = self.input_handler.select_option(
                    title="Settings & Help",
                    options=[("settings", "Settings", "s"), ("help", "Help & Glossary", "h"), ("back", "Back", "b")],
            ...
            if self.input_handler.get_input("proceed_prompt", "y", ["y", "n"]) != "y":
                try:
                    args = self.collector.collect_parameters(interactive=True, base_args=normalized)
                    normalized = validate_parameters(apply_defaults(args))
                    self.display_manager.display_params_summary(normalized)
                except Exception:
                    self.console.print(MESSAGES.get("params_discarded", "Parameters discarded"))
                    continue
            ...
            try:
                ...
                experiment_params = {k: v for k, v in normalized.items() if k not in ["name", "description", "category", "difficulty"]}
    ...
    def _show_help_menu(self) -> None:
        ...
        items = glossary.items() if not term else [(k, v) for k, v in glossary.items() if term.lower() in k.lower() or term.lower() in v.lower()]
        if not items:
