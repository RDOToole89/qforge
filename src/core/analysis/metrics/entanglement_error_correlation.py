...
from ..constants import (
    ALPHA, EEC_LAMBDA, validate_counts_dict,
    CORRELATION_WEAK_THRESHOLD, CORRELATION_MODERATE_THRESHOLD, CORRELATION_STRONG_THRESHOLD
)
from ..core.information_theory import counts_to_probabilities, mutual_information
...
def compute_entanglement_error_correlation(counts: Mapping[str, int],
                                         state_type: str = "GHZ",
    ...
    if not counts_clean:
        ...
        return 0.0 if not return_analysis else TopologyAnalysis(
    ...
    logger.debug(f"Computing EEC for {n_qubits}-qubit {state_type} state with {len(counts_clean)} outcomes")
    ...
    error_matrix = _compute_error_correlation_matrix(counts_clean, n_qubits)
    ...
    return _generate_topology_analysis(
        eec, p_value, entanglement_matrix, error_matrix, 
        state_type, counts_clean, n_qubits
    )


def compute_multiway_entanglement_correlation(counts: Mapping[str, int],
                                            state_type: str = "GHZ",
                                            max_order: int = 3) -> Dict[int, float]:
    """
...
def _construct_entanglement_topology(n_qubits: int, 
                                   state_type: str,
...
def _compute_error_correlation_matrix(counts: dict, n_qubits: int) -> np.ndarray:
    """
    Compute error correlation matrix from measurement data.
    ...
    for i in range(n_qubits):
        for j in range(i + 1, n_qubits):
            try:
                ...
                mi = mutual_information(counts, i, j)
                E[i, j] = E[j, i] = mi
            except Exception as e:
                logger.debug(f"Failed to compute MI for qubits ({i},{j}): {e}")
                E[i, j] = E[j, i] = 0.0
    
    # Alternative: Direct frequency correlation
    # Could also compute correlation of error frequencies directly
    
    logger.debug(f"Computed error correlation matrix with mean MI = {np.mean(E[E > 0]):.4f}")
    return E


def _compute_topology_error_correlation(entanglement_matrix: np.ndarray,
                                      error_matrix: np.ndarray) -> Tuple[float, float]:
    """
    ...
    if len(set(entanglement_flat)) <= 1 or len(set(error_flat)) <= 1:
...
def _compute_kway_entanglement_weight(qubit_combo: Tuple[int, ...],
                                    state_type: str,
                                    n_qubits: int) -> float:
...
def _compute_kway_error_frequency(qubit_combo: Tuple[int, ...],
                                counts: dict) -> float:
...
def _generate_topology_analysis(eec: float,
                              p_value: float,
                              entanglement_matrix: np.ndarray,
                              error_matrix: np.ndarray,
                              state_type: str,
                              counts: dict,
                              n_qubits: int) -> TopologyAnalysis:
    ...
    top_indices = np.argsort(weights)[-max(1, len(weights) // 4):]
    ...
    summary = (f"EEC = {eec:.3f} ({strength} {direction} correlation, "
...
def validate_eec_properties(eec: float,
...
def entanglement_error_correlation_educational_demo() -> dict:
    ...
    # GHZ state examples
    ghz_structured = {"000": 400, "111": 400, "001": 100, "110": 100}
    ghz_random = {"000": 250, "001": 250, "010": 250, "011": 125, "others": 125}
    ...
    w_counts = {"001": 300, "010": 300, "100": 300, "000": 100, "111": 0}
    ...
    demo_results["state_comparison"] = {
        ...
        "w": {"eec": eec_w, "interpretation": "Single-excitation manifold preservation"},
