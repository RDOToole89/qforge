...
def compute_pathway_concentration_ratio(counts: Mapping[str, int],
    ...
    Examples:
        ...
        >>> counts = {"000": 400, "111": 300, "001": 100, "010": 50, "others": 150}
        >>> compute_pathway_concentration_ratio(counts)
        14.0
    ...
    if not counts_clean:
        ...
        return 1.0 if not return_analysis else ConcentrationAnalysis(
    ...
    logger.debug(f"Computed PCR = {pcr:.3f} (top: {top_quartile_sum}, bottom: {bottom_quartile_sum})")
...
def compute_concentration_with_gini(counts: Mapping[str, int]) -> Tuple[float, float]:
    ...
    Examples:
        >>> counts = {"000": 400, "111": 300, "010": 200, "others": 100}
    ...
    frequencies = sorted(counts_clean.values())
    n = len(frequencies)
    ...
    if n <= 1:
        ...
        gini = (2.0 * np.sum(np.arange(1, n + 1) * frequencies)) / (n * cumsum[-1]) - (n + 1) / n
...
def _generate_concentration_analysis(pcr: float,
                                   frequencies: List[int],
                                   counts_clean: dict,
                                   top_k: int,
                                   bottom_k: int,
                                   total_counts: int) -> ConcentrationAnalysis:
    ...
    if n > 1:
        ...
        gini = (2.0 * np.sum(np.arange(1, n + 1) * sorted(frequencies))) / (n * cumsum[-1]) - (n + 1) / n
        gini = max(0.0, gini)
    else:
        gini = 0.0
    ...
    # Identify dominant and rare pathways
    sorted_outcomes = sorted(counts_clean.items(), key=lambda x: x[1], reverse=True)
    ...
    if pcr == float('inf'):
        ...
        summary = (f"PCR={pcr:.2f} ({concentration_evidence} concentration): "
...
def validate_pcr_properties(pcr: float,
...
def pathway_concentration_educational_demo() -> dict:
    ...
    high_counts = {"000": 600, "111": 300, "001": 50, "010": 30, "others": 20}
    ...
    extreme_counts = {"0000": 900, "0001": 50, "0010": 25, "0011": 15, "others": 10}
