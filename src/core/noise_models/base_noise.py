...
class BaseNoise(ABC):
    ...
    @abstractmethod
    def apply(self, noise_model: NoiseModel, gate_list: List[str], qubits_for_error: Optional[int] = None) -> None:
        """
    ...
    def validate_for_hardware(self, backend_constraints: Dict[str, Any]) -> List[str]:
        ...
        if supported_gates:
            ...
            if unsupported:
                warnings.append(
                    f"Noise requires unsupported gates: {list(unsupported)}"
                )
    ...
    def _validate_error_rate(self, error_rate: float) -> None:
        ...
        if not 0 <= error_rate <= 1:
            raise ValueError(
                f"Error rate must be probability in [0,1], got {error_rate}"
            )
    ...
    def log_noise_creation(self, noise_type: str, extra_info: Optional[Dict] = None) -> None:
        """
