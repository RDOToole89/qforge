...
class DepolarizingNoise(BaseNoise):
    ...
    def apply(self, noise_model: NoiseModel, gate_list: List[str], qubits_for_error: int = None) -> None:
        ...
        """
        # Determine effective qubit count for error application
        effective_qubits = qubits_for_error if qubits_for_error is not None else self.num_qubits
        
        # Validate qubit count for depolarizing channel construction
        if effective_qubits < 1 or effective_qubits > 5:
            raise ValueError(
                f"Depolarizing noise supports 1-5 qubits, got {effective_qubits}. "
                f"Larger systems require specialized tensor product construction."
            )
        
        # Create depolarizing error channel with physics validation
        try:
            depolarizing_channel = depolarizing_error(self.error_rate, effective_qubits)
        except Exception as e:
            raise ValueError(
                f"Failed to create {effective_qubits}-qubit depolarizing channel "
                f"with error rate {self.error_rate:.4f}: {e}"
            ) from e
        
        # Apply depolarizing noise to specified gates
        successful_gates = []
        failed_gates = []
        ...
        for gate in gate_list:
            try:
                ...
                noise_model.add_all_qubit_quantum_error(depolarizing_channel, gate)
        ...
        if successful_gates:
            logger.info(
                ...
                f"{effective_qubits}-qubit gates: {successful_gates} "
        ...
        logger.debug(
            ...
            f"effective_qubits={effective_qubits}"
        )
