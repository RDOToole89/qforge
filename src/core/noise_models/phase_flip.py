...
class PhaseFlipNoise(BaseNoise):
    ...
    def apply(self, noise_model: NoiseModel, gate_list: List[str], qubits_for_error: int = None) -> None:
        ...
        if failed_gates:
            logger.warning(
                f"Failed to apply PHASE_FLIP noise to gates: "
    ...
    def get_physics_description(self) -> Dict[str, str]:
        ...
        return {
            ...
            "magnetic_noise": f"Magnetic field noise = {self.magnetic_field_noise:.2e}T" if self.magnetic_field_noise else "phenomenological rate",
            "charge_noise": f"Charge noise = {self.charge_noise:.2e}eV" if self.charge_noise else "no charge coupling",
    ...
    def _validate_phase_flip_params(
        ...
        if not 0 <= error_rate <= 1:
            raise ValueError(
                f"Phase flip rate must be in [0,1], got {error_rate}"
            )
        
        # Validate magnetic field noise if provided
        if magnetic_field_noise is not None and magnetic_field_noise < 0:
            raise ValueError(f"Magnetic field noise must be non-negative, got {magnetic_field_noise}")
    ...
    def _get_gate_sensitivity_map(self) -> Dict[str, float]:
        ...
        return {
            ...
            'z': 0.1, 'rz': 0.1, 'u1': 0.1, 'p': 0.1, 's': 0.1, 't': 0.1, 'sdg': 0.1, 'tdg': 0.1,
    ...
    def __str__(self) -> str:
        ...
        if self.magnetic_field_noise or self.charge_noise:
            ...
            return (
                f"Phase flip: p={self.error_rate:.4f} "
                f"[phenomenological longitudinal error]"
            )