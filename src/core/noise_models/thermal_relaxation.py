...
class ThermalRelaxationNoise(BaseNoise):
    ...
    def get_kraus_operators(self) -> List[np.ndarray]:
        ...
        gamma_phi = self._t2_error_rate - gamma_1/2  # Pure dephasing rate
    ...
    def _get_gate_times(self) -> Dict[str, float]:
        ...
        return {
            ...
            'x': self.gate_time, 'y': self.gate_time, 'h': self.gate_time,
            'rx': self.gate_time, 'ry': self.gate_time,
            'u2': self.gate_time, 'u3': self.gate_time, 'u': self.gate_time,
            
            # Two-qubit gates (longer duration)
            'cx': 2 * self.gate_time, 'cy': 2 * self.gate_time, 'cz': 2 * self.gate_time,
            'ch': 2 * self.gate_time, 'swap': 3 * self.gate_time, 'iswap': 3 * self.gate_time
        }
