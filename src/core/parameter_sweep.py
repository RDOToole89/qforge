...
class ParameterSweepEngine:
    ...
    def __init__(self, results_dir: str = "results"):
        ...
        logger.info(
            f"Parameter sweep engine initialized with results directory: {self.results_dir}"
        )
    ...
    def run_parameter_sweep(
        ...
        if sweep_name is None:
            sweep_name = (
                f"{base_experiment_id}_sweep_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
        ...
        # Progress (TTY) unless quiet/JSON
        use_rich = not bool(os.getenv("QEXP_QUIET")) and not bool(
            os.getenv("QEXP_JSON_ONLY")
        )
        progress_ctx = (
            Progress(
                SpinnerColumn(),
        ...
        if progress_ctx:
            progress_ctx.start()
            task = progress_ctx.add_task("Sweeping parameters", total=total_experiments)
        ...
        for param_combo in param_combinations:
            ...
            try:
                ...
                logger_utils.log_with_experiment_id(
                    ...
                    "combo_start",
                    sweep_id,
                    extra_info={"parameters": param_combo},
                )
            except Exception:
                pass

            combo_results = {"parameter_combination": param_combo, "runs": []}

            # Run multiple times for statistical robustness
            for run_num in range(runs_per_config):
                experiment_count += 1
                logger.info(
                    f"   Run {run_num + 1}/{runs_per_config} ({experiment_count}/{total_experiments})"
                )

                try:
                    # Run the experiment with custom parameters
                    experiment_id = (
                        f"{base_experiment_id}_sweep_{sweep_id}_{experiment_count}"
                    )
                    result = em.run_experiment(
                        base_experiment_id, custom_params=param_combo
                    )

                    if (
                        result
                        and isinstance(result, dict)
                        and "research_analysis" in result
                    ):
                        run_result = {
                        ...
                        try:
                            logger_utils.log_with_experiment_id(
                                ...
                                "run_success",
                                experiment_id,
                                extra_info={"run": run_num + 1},
                            )
                        except Exception:
                            pass
                    else:
                        logger.error(
                            f"   ❌ Run {run_num + 1} failed - no research analysis returned"
                        )
                    ...
                    try:
                        logger_utils.log_with_experiment_id(
                            ...
                            "run_failure",
                            f"{base_experiment_id}_sweep_{sweep_id}_{experiment_count}",
                            extra_info={"run": run_num + 1, "error": str(e)},
                        )
                    except Exception:
                        pass
        ...
        sweep_results["aggregated_analysis"] = self._aggregate_sweep_results(
            sweep_results["experiment_results"]
        )
    ...
    def _aggregate_sweep_results(
        ...
        for combo_result in experiment_results:
            ...
            successful_runs = [
                run for run in combo_result["runs"] if run.get("success", False)
            ]
        ...
        for combo_result in experiment_results:
            ...
            if successful_runs:
                ...
                avg_entropy = sum(
                    run["research_analysis"]["research_metrics"]["information_theory"][
                        "normalized_entropy"
                    ]
                    for run in successful_runs
                ) / len(successful_runs)
                ...
                avg_kl_div = (
                    sum(
                        run["research_analysis"]["research_metrics"][
                    ...
                    in successful_runs[0]["research_analysis"]["research_metrics"]
                    else 0
                )
                ...
                aggregated["parameter_analysis"][param_key] = {
                    ...
                    "entropy_std": self._calculate_std(
                        [
                            run["research_analysis"]["research_metrics"][
                                "information_theory"
                            ]["normalized_entropy"]
        ...
        if len(all_successful_runs) > 1:
            entropies = [
                run["research_metrics"]["information_theory"]["normalized_entropy"]
                for run in all_successful_runs
            ]
            min_entropy = min(entropies)
            max_entropy = max(entropies)

            aggregated["key_findings"].append(
                f"Entropy range: {min_entropy:.3f} - {max_entropy:.3f}"
            )

            if max_entropy - min_entropy > 0.1:
                aggregated["key_findings"].append(
                    "Significant entropy variation detected across parameters"
                )
        ...
        aggregated["statistical_summary"] = {
            ...
            "success_rate": (
                len(all_successful_runs)
                / sum(len(combo["runs"]) for combo in experiment_results)
    ...
    def _save_sweep_results(
        self, sweep_results: Dict[str, Any], sweep_name: str
    ) -> str:
        """Save comprehensive sweep results."""
...
def run_noise_sweep(
    experiment_id: str, noise_levels: List[float] = None, runs_per_level: int = 3
) -> Dict[str, Any]:
...
def run_convergence_test(
    experiment_id: str, shot_counts: List[int] = None, runs_per_count: int = 3
) -> Dict[str, Any]:
...
def run_custom_sweep(
    experiment_id: str, parameter_ranges: Dict[str, List[Any]], runs_per_config: int = 3
) -> Dict[str, Any]:
