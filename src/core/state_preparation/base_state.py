...
class BaseState(ABC):
    ...
    def validate_for_hardware(self, backend_constraints: Dict[str, Any]) -> List[str]:
        ...
        if self.num_qubits > max_qubits:
            warnings.append(
                f"State requires {self.num_qubits} qubits, "
                f"hardware supports maximum {max_qubits}"
            )
        ...
        if max_depth is not None:
            ...
            if estimated_depth > max_depth:
                warnings.append(
                    f"Estimated circuit depth {estimated_depth} exceeds "
                    f"hardware limit {max_depth}"
                )
        ...
        if supported_gates is not None:
            ...
            if unsupported:
                warnings.append(
                    f"State requires unsupported gates: {list(unsupported)}"
                )
