...
class BellState(BaseState):
    ...
    def __init__(self, num_qubits: int, custom_params: Dict = None, experiment_id: str = "N/A"):
        ...
        if variant not in valid_variants:
            raise ValueError(
                f"Invalid Bell variant: '{variant}'. "
    ...
    def create(self, add_barrier: bool = False) -> QuantumCircuit:
        ...
        self.log_state_creation(
            f"Bell {variant.upper()}",
            {
                ...
                "measurement_correlations": "perfect_anti-correlation" if "minus" in variant else "perfect_correlation"
            }
        )
    ...
    def get_theoretical_properties(self) -> Dict[str, Any]:
        ...
        if variant in ["phi_plus", "phi_minus"]:
            ...
            correlation_type = "perfect correlation" if variant == "phi_plus" else "perfect anti-correlation"
        else:  # psi_plus, psi_minus
            measurement_basis = "superposition (X-basis)"
            correlation_type = "perfect correlation" if variant == "psi_plus" else "perfect anti-correlation"
