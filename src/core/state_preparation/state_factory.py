...
def prepare_state(
    ...
    try:
        ...
        logger.info(
            f"Created {state_type} state: {num_qubits} qubits "
            f"(experiment: {experiment_id})"
        )
        logger.debug(
            f"State details: {state_type}, qubits={num_qubits}, "
        ...
        raise ValueError(
            f"Failed to create {state_type} state: {e}"
        ) from e
    ...
    if backend is not None:
        try:
            ...
            backend_constraints = {
                ...
                'gate_set': set(backend_config.basis_gates) if hasattr(backend_config, 'basis_gates') else set(),
