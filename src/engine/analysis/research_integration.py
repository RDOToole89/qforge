...
from src.core.analysis.metrics.concentration_index import compute_concentration_index
...
def extract_counts_from_result(raw_result: Any) -> Dict[str, int]:
    ...
    try:
        if hasattr(raw_result, "get_counts"):
            ...
            # Convert Qiskit Counts object to clean bitstring format
            clean_counts = {}
            for bitstring, count in counts_raw.items():
                # Remove spaces and convert to clean format
                clean_bitstring = bitstring.replace(" ", "")
                clean_counts[clean_bitstring] = count
            return clean_counts
        elif isinstance(raw_result, dict) and "counts" in raw_result:
            counts_obj = raw_result["counts"]
            # Convert to clean bitstring format
            clean_counts = {}
            for bitstring, count in counts_obj.items():
                clean_bitstring = bitstring.replace(" ", "")
                clean_counts[clean_bitstring] = count
            return clean_counts
            ...
            logger.warning(
                "Density matrix results not supported for structured decoherence metrics"
            )
            return {}
        else:
...
def compute_research_metrics(
    ...
    try:
        ...
        # Compute all 8 structured decoherence metrics
        ...
        # Compute new schema-required metrics
        # For now, use asymmetry index as structure score (they're related)
        ss = ai  # Structure score is similar to asymmetry index

        ci = compute_concentration_index(counts)

        tc_result = compute_total_correlation(counts=counts)
        tc = tc_result.get("value", 0.0) if isinstance(tc_result, dict) else 0.0
        ...
        logger.info(
            f"Research metrics computed: AI={ai:.4f}, PCR={pcr:.4f}, EEC={eec:.4f}"
        )
        return metrics
