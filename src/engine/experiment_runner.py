...
class EngineExperimentRunner:
    ...
    def _apply_noise(
        ...
        try:
            ...
            # Store noise model for simulation (Qiskit handles noise application)
            self.noise_model = noise_model

            self.logger.info(
                f"Created {noise_type} noise model with parameters: {noise_params}"
            )
            return circuit
    ...
    def _execute_simulation(
        ...
        if rng_seed is not None:
            backend.set_options(seed_simulator=rng_seed)

        # Use noise model if available
        if hasattr(self, "noise_model") and self.noise_model:
            job = backend.run(circuit, shots=shots, noise_model=self.noise_model)
        else:
