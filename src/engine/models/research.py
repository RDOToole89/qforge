...
class StructuredDecoherenceMetrics(BaseModel):
    ...
    def is_structured(self) -> bool:
        ...
        return (
            self.asymmetry_index > 0.3 and 
            self.pathway_concentration_ratio > 1.5 and
            abs(self.entanglement_error_correlation) > 0.3
        )
...
class AnalysisMetadata(BaseModel):
    ...
    noise_conditions: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Noise model parameters used"
    )
...
class PathwayAnalysis(BaseModel):
    ...
    research_notes: Optional[str] = Field(
        default=None,
        description="Additional research insights or observations"
    )
    ...
    def validate_asymmetry_level(cls, v: str) -> str:
        ...
        valid_levels = ["very_uniform", "slight_asymmetry", "moderate_asymmetry", "high_asymmetry"]
        if v not in valid_levels:
    ...
    def validate_entanglement_influence(cls, v: str) -> str:
        ...
        valid_levels = ["no_correlation", "weak_correlation", "moderate_correlation", "strong_correlation"]
        if v not in valid_levels:
...
class ResearchMetadata(BaseModel):
    ...
    expected_outcomes: Optional[List[str]] = Field(
        default=None,
        description="Expected experimental outcomes or predictions"
    )
    
    control_experiment: bool = Field(
        default=False,
        description="Whether this is a control experiment"
    )
    
    # Publication metadata  
    publication_ready: bool = Field(
        default=False,
        description="Whether results meet publication quality standards"
    )
...
class ComparisonMetrics(BaseModel):
    ...
    delta_asymmetry_index: Optional[float] = Field(
        default=None,
        description="Change in AI relative to baseline"
    )
    
    delta_pathway_concentration: Optional[float] = Field(
        default=None,
        description="Change in PCR relative to baseline"
    )
    
    delta_entanglement_correlation: Optional[float] = Field(
        default=None,
        description="Change in EEC relative to baseline"
    )
    ...
    effect_size: Optional[float] = Field(
        default=None,
        description="Effect size (Cohen's d) for practical significance"
    )
    
    # Trend analysis
    trend_direction: Optional[str] = Field(
        default=None,
        description="Overall trend direction across conditions"
    )
    ...
    def validate_trend_direction(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            valid_trends = ["increasing", "decreasing", "stable", "nonlinear", "unclear"]
