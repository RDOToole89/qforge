...
def test_generate_html_report_from_results(tmp_path):
    analysis = {
        "schema_version": "1.0.0",
        "experiment_metadata": {
            "experiment_id": "abc12345",
        ...
        "research_metrics": {"information_theory": {"normalized_entropy": 0.5}},
        "provenance": {"software_versions": {"python": "3.13"}},
    }
    json_path = tmp_path / "analysis.json"
    json_path.write_text(json.dumps(analysis))
    ...
    out = save_report_from_json(str(json_path), fmt="html")
    outp = Path(out)
    assert outp.exists()
    html = outp.read_text().lower()
    assert "<html" in html and "experiment report" in html
